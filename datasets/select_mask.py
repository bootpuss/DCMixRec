...
def define_Mask(opt):
    ...
    # 320 * 320 Gaussian 1D
    elif mask_name == 'G1D30_320':
        mask = loadmat(os.path.join('mask_320', 'G1D30.mat'))['maskRS1']

    # 320 * 320 Gaussian 2D
    elif mask_name == 'G2D30_320':
        mask = loadmat(os.path.join('mask_320', 'G2D30.mat'))['maskRS2']

    # 320 * 320 radial
    elif mask_name == 'R30_320':
        mask_shift = cv2.imread(os.path.join('mask_320', 'R30.tif'), 0) / 255
        mask = scipy.fftpack.fftshift(mask_shift)

    # 320 * 320 spiral
    elif mask_name == 'S30_320':
        mask_shift = cv2.imread(os.path.join('mask_320', 'S30.tif'), 0) / 255
        mask = scipy.fftpack.fftshift(mask_shift)

    # 320 * 320 Gaussian 1D
    elif mask_name == 'G1D10_320':
        mask = loadmat(os.path.join('mask_320', 'G1D10.mat'))['maskRS1']

    # 320 * 320 Gaussian 2D
    elif mask_name == 'G2D10_320':
        mask = loadmat(os.path.join('mask_320', 'G2D10.mat'))['maskRS2']

    # 320 * 320 radial
    elif mask_name == 'R10_320':
        mask_shift = cv2.imread(os.path.join('mask_320', 'R10.tif'), 0) / 255
        mask = scipy.fftpack.fftshift(mask_shift)

    # 320 * 320 spiral
    elif mask_name == 'S10_320':
        mask_shift = cv2.imread(os.path.join('mask_320', 'S10.tif'), 0) / 255
        mask = scipy.fftpack.fftshift(mask_shift)
